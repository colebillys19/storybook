...
import { StyledBaseTextInput } from './styles/EmailInput.styles';
...
const EmailInput = ({
  ...
  const validateInput = (e) => {
    if (e.target.value && !isEmail(e.target.value, validatorOptions) && isValid) {
      onInvalidInput();
      toggleValidity(false);
    } else if (e.target.value && isEmail(e.target.value, validatorOptions) && !isValid) {
      onValidInput();
      toggleValidity(true);
    }
  };
...
EmailInput.propTypes = {
  ...
  /** Function to be run when user input is not valid email format. Validity will be checked
  ...
  /** Function to be run when user input changes from valid to invalid email format. Validity will
